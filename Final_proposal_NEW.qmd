...
ggplot(dates, aes(x = CRASH.DATE, y = Count)) +
  ...
  scale_x_date(date_labels = "%Y", breaks = seq(min(dates$CRASH.DATE), max(dates$CRASH.DATE), by = "1 year"))
...
merged_data3 <- merge(daily_counts, weatherdata, by.x="CRASH.DATE", by.y="datetime")
...
merged_data <- merge(nyc_crashes, weatherdata, by.x="CRASH.DATE", by.y="datetime")
...
ggplot(data = merged_data, aes(x = as.factor(icon),
...
nyc_crashes <- nyc_crashes %>%
  mutate(WEEKDAY = weekdays(CRASH.DATE)) %>%
  filter(BOROUGH!="")
