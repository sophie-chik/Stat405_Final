...
```{r Regression plot, fig.cap="Total Number of Car Crashes vs Precipitation per Day in NYC"}
...
The heat map shows that in all boroughs, the highest number of crashes occurs on Fridays. The lowest in all boroughs occurs on Sundays. In general, a higher number of crashes happen on weekdays, which makes sense since a lot of New York City residents who work drive to and from work on weekdays. The plot also demonstrates variation in car crashes per borough. Staten Island has the least variation, but also has the least number of crashes as we saw in the "Number of Car Crashes and Deaths per Borough" plot. Manhattan has the highest variation, followed by Brooklyn.
